...
;!function () {
    ...
    L.SGMarkers = L.LayerGroup.extend({
	includes: L.Mixin.Events,
	...
	updateMarkers : function (markers) {
		...
		//IE8 does not have forEach :(
		for (var i = 0; i < markers.length; i++) {
			var marker = markers[i];
			marker.addTo(that._map);
		}
		this.marker_div.appendChild(this.fragment);
	},

	setLayerVisibility: function (visible) {
		//If at the section level, operate as a block
		if (this.type == "section") {
			this.marker_div.style.display = visible ? "block" : "none";
		}
		//Otherwise only render markers on screen
		else if (this.type == "row") {
			if (visible) {
				var that = this;
				var map_bounds = that._map.getBounds();

				for (var i = 0; i < this.markers.length; i++) {
					var marker = this.markers[i];
					var latlng = marker.actual_latlng;
					marker.changeVisibility(map_bounds.contains(latlng));
				}
			}
			this.marker_div.style.display = visible ? "block" : "none";
		}
	...
	updateTooltipFormatter: function () {
		for (var i = 0; i < this.markers.length; i++) {
			var marker = this.markers[i];
		}
	},

	animateTransition: function (direction) {
		for (var i = 0; i < this.markers.length; i++) {
			var marker = this.markers[i];
			if (L.DomUtil.TRANSITION) {
			  if (marker._icon) { marker._icon.style[L.DomUtil.TRANSITION] = ('all 0.2s cubic-bezier(0,0,0.25,1)'); }
			}
			if (marker.parent_latlng) {
				if (direction == "apart") {
					marker.setLatLng(marker.actual_latlng);
					marker._icon.style.opacity = "1";
				}
				else if (direction == "together") {
					marker.setLatLng(marker.parent_latlng);
					marker._icon.style.opacity = ".6"; //Reduce additive effect of shadows
				}
			}
		}
	},
	...
	onAdd: function (map) {
		this._map = map;
		this._map._panes.markerPane.appendChild(this.marker_div);
	},
	...
	getByMapKey: function (mk) {
		var output_marker;
		var parsed_key = sgmap.parseMapKey(mk);
		var search_term = "";
		if (this.type == "section") {
			search_term = "s:" + parsed_key.s;
		}
		else {
			search_term = mk;
		}
		for (var i = 0; i < this.markers.length; i++) {
			var marker = this.markers[i];
			if (search_term == marker.data.mapkey) {
...
L.sgmarkers = function (name) {
	return new L.SGMarkers(name);
};

;sgmap.SGMAP_URL = "http://a.tiles.seatgeek.com/v3/maps";
sgmap.SGLISTINGS_URL = "http://seatgeek.com/event/tickets";
sgmap.SGSANDCRAB_URL = "http://seatgeek.com/event/sandcrab";
sgmap.SGAPI_URL = "http://api.seatgeek.com/2";
sgmap.SGAPI_CLIENT_ID = "MTY2MnwxMzgzMzIwMTU4";

L.CRS.SeatGeek = L.extend({}, L.CRS.Simple, {
  transformation: new L.Transformation(0.35, 0, 0.35, 0)
});

sgmap.Map = L.Map.extend({

  options: {
    center: [500, 500], //TODO: CHANGE BACK TO 500
    zoom: 1,
    minZoom: 1,
    maxZoom: 4,
    crs: L.CRS.SeatGeek,
    ...
    // Border/gutter to be used in calculations sensitive to the map edge.
    border: { top: 60, left: 0, bottom: 60, right: 0 },
    // Tile size for the default tile layer
    tileSize: 350,
  ...
  _firstLoad: true,
  block_popup: false,
  _selected_section: null, //Current selected row or section

  //NOTE: 383px is the width of the omnibox + padding (368 + 15)
  // Bing only has 5px of padding, so omnibox is 373px (368 + 5)
  // 70px is the height of the ad bar (which does not exist on Bing)
  initialize: function (element, mapid, options) {
    L.Map.prototype.initialize.call(this, element, options);

    var ad_height = IS_BING_INTEGRATION ? 0 : 70;
    var box_width = IS_BING_INTEGRATION ? 373 : 383;
    var targetLatLng = [500, 500];
    var targetZoom = this.options.zoom;
    var targetPoint = this.project(targetLatLng, targetZoom).subtract([box_width/2, -1*ad_height/2]),
        targetLatLng = this.unproject(targetPoint, targetZoom);

    this.setView(targetLatLng, targetZoom, {animate : false});
    var that = this;
  ...
  _loadMapData: function (mapid) {
    ...
    $.ajax({
      ...
      success: function (map_data) {
        // TODO: this should respect the templates in map_data.json
        var retina = window.devicePixelRatio > 1;
        var normal_url = "http://{s}.tiles.seatgeek.com/v3/maps/{mapId}/{z}/{x}/{y}.png";
        var retina_url = (mapid.indexOf("retina") === -1) ? "http://{s}.tiles.seatgeek.com/v3/maps/{mapId}.retina/{z}/{x}/{y}.png" : normal_url;
        var tile_url = retina ? retina_url : normal_url;
        L.tileLayer(tile_url, {
          mapId: mapid,
          tileSize: that.options.tileSize,
          minZoom: 1,
          maxZoom: 4,
          noWrap: true,
          detectRetina: true,
        ...
        that._mapData = map_data;

        _.defer(function () {
          that.fire("map-data:load");
        });
      }
    });
  },
  ...
  // ********** Extensions of Map.ZoomAnimation.js **************
  _tryAnimatedZoom: function (center, zoom, options) {
    if (this._animatingZoom || this._fullZoom) { return true; }
  ...
  _onZoomTransitionEnd: function () {
    ...
    var delay = 0;
    var switch_level = this._switchLevel || 2;

    if (this._animateToZoom == switch_level && this._scale > 1) {
      delay = 500;
    }
  ...
  _mapDetail: function (zoom, switch_level) {
    return zoom < switch_level ? "section" : "row";
  },

  _emptyMarkers: function () {
    this.sectionMarkerLayer.empty();
    this.rowMarkerLayer.empty();
  },

  _initMarkers: function(marker_info, event_view) {
    var that = this;
    var markers = _.map(_.sortBy(marker_info, "sort"), function (data) {
        var location = that.getSectionOrRowForMapKey(data.mapkey),
            marker = that._getMarkerBySection(location, data, event_view);

        var section = that.getSectionForMapKey(data.mapkey);

        marker.actual_latlng = marker._latlng;
        marker.data = data;
        marker.marker_formatter = that.marker_formatter;
        marker.bindPopup(that.popup_formatter);
        marker.bindTooltip(that.tooltip_formatter);

        if (section != location) {
          var parent_marker = that._getMarkerBySection(section, data, event_view);
          marker.parent_latlng = parent_marker._latlng;
          if (that.map_detail == "section") {
            marker.setLatLng(marker.parent_latlng);
          }
        }
        return marker
  ...
  _getMarkerBySection: function(section, data, event_view) {
    var that = this;
    var scale_factor = event_view.scale_factor;
    var marker = new sgmap.Marker([section.center[1] * scale_factor, section.center[0] * scale_factor], {
      icon: that.marker_formatter(data)
    });
  ...
  updateFormatters: function() {
    if (this.rowMarkerLayer) {
      this.rowMarkerLayer.updateTooltipFormatter();
    }
    if (this.sectionMarkerLayer) {
      this.sectionMarkerLayer.updateTooltipFormatter();
    }
  },

  updateMarkers: function (event_view, zoom, listings) {
    this.event_view = event_view;
    if (!this.sectionMarkerLayer) {
      this.sectionMarkerLayer = L.sgmarkers("section").addTo(this);
    }

    if (!this.rowMarkerLayer) {
      this.rowMarkerLayer = L.sgmarkers("row").addTo(this);
    }

    var section_marker_info = listings.getSectionMarkerInfo();
    var row_marker_info = listings.getRowMarkerInfo();

    if (!this._firstLoad) {
      this._emptyMarkers();
    }

    this.section_markers = this._initMarkers(section_marker_info, event_view);

    this.row_markers = this._initMarkers(row_marker_info, event_view);

    var switch_level = event_view.switch_level;

    this.sectionMarkerLayer.updateMarkers(this.section_markers);
    this.rowMarkerLayer.updateMarkers(this.row_markers);
    this._updateVisibleMarkers(zoom, switch_level);

    this._firstLoad = false;
  },

  _updateVisibleMarkers: function (zoom, switch_level) {
    var detail = this._mapDetail(zoom, switch_level);
    var last_detail = this.map_detail;
    //switch views
    this.current_layer = detail;

    //SECTION VIEW
    if (detail == "section") {
      //ROW LAYER
      if (this.rowMarkerLayer) {
        //ROW -> SECTION ANIMATION
        if (last_detail != detail) {
          this.fire("map:detail-change");
          this.rowMarkerLayer.animateTransition("together");
          var that = this;
          setTimeout(function() {
            that.rowMarkerLayer.setLayerVisibility(false);
          }, 125);
        }
        else {
          this.rowMarkerLayer.setLayerVisibility(false);
        }
      }
      //SECTION LAYER
      if (this.sectionMarkerLayer) {
        this.sectionMarkerLayer.setLayerVisibility(true);
      }
    }
    // ROW VIEW
    else {
      //ROW LAYER
      if (this.rowMarkerLayer) {
        this.rowMarkerLayer.setLayerVisibility(true);
        if (last_detail != detail) {
          var that = this;
          this.fire("map:detail-change");
          setTimeout(function() {
            that.rowMarkerLayer.animateTransition("apart");
          }, 125);
        }
      }
      //SECTION LAYER
      if (this.sectionMarkerLayer) {
        this.sectionMarkerLayer.setLayerVisibility(false);
      }
    }
    this.map_detail = detail;
  },

  getSectionOrRowForMapKey: function (mk) {
    mk = sgmap.parseMapKey(mk);
    var section = this._mapData.sections[mk.s];
    if (mk.r && section.rows && section.rows[mk.r]) {
      return section.rows[mk.r];
    }
    return section;
  },

  getSectionForMapKey: function (mk) {
    mk = sgmap.parseMapKey(mk);
    var section = this._mapData.sections[mk.s];
    return section;
  },

  getFieldCenter: function () {
    return new L.LatLng(
        this._mapData.sections._center[1],
        this._mapData.sections._center[0]
    );
  }

});

sgmap.map = function (element, mapid, options) {
  return new sgmap.Map(element, mapid, options);
};

;sgmap.Marker = L.Marker.extend({
  ...
  _initIcon: function () {
    ...
    if (addIcon) {
      var current_frag = this._map.current_layer + "_frag";
      this._map[current_frag].appendChild(this._icon);
    }
  },

  changeVisibility: function (visible) {
    this._icon.style.display = visible ? "block" : "none";
  },

  _updateZIndex: function (offset) {
    this._icon.style.zIndex = 0 + offset;
  },
  ...
  _onMarkerClick: function() {
    this.unsetSelected();
    this.setSelected();
    this._map.event_view.trigger("marker:selected", this.data.mapkey);
  },

  setSelected: function () {
    //If another section is selected, clear
    var points = sgmap.pathToPoints(app.map.getSectionOrRowForMapKey(this.data.mapkey).path);
    this._selected_highlight = new L.multiPolygon(points, {
      weight: 3.5,
      color: "#007bda",
      opacity: 0.95,
      fillOpacity: 0.0
    }).addTo(this._map);
    this._map.selected_marker = this;
  },

  unsetSelected: function () {
    if (this._map.selected_marker) {
      this._map.removeLayer(this._map.selected_marker._selected_highlight);
      this._map.selected_marker._selected_highlight = null;
      this._map.selected_marker = null;
    }
  },

  openTooltip: function () {
    //Check if tooltip should be blocked (during zoom)
    if (this._map._fullZoom) {
      return;
    }
    if (!this._tooltip) {
      var content = this.tooltipFormatter(this),
          options = {
            offset: L.point([0, -8]),
            autoPan: false,
            closeButton: false,
      ...
      this._tooltip = new sgmap.Tooltip(options, this)
        .setContent(content);
    }
    if (this._map.options.enableHighlights && !this._highlight && !this._selected_highlight) {
      var points = sgmap.pathToPoints(app.map.getSectionOrRowForMapKey(this.data.mapkey).path);
      this._highlight = new L.multiPolygon(points, {
        weight: 3.5,
        color: "#007bda",
        opacity: 0.45,
        fillOpacity: 0
    ...
    if (this._tooltip && this._map && !this._map.hasLayer(this._tooltip)) {
      this._tooltip.setLatLng(this._latlng);
      this._map.openPopup(this._tooltip);
    }
    return this;
  },
  ...
  bindTooltip: function (tooltipFormatter) {
    if (!this._tooltipHandlersAdded) {
      this
          .on('mouseover', this.openTooltip, this)
          .on('mouseout', this.closeTooltip, this)
          .on('remove', this.closeTooltip, this)
  ...
  unbindTooltip: function () {
    if (this._tooltip) {
      this._tooltip = null;
      this
          .off('mouseover', this.openTooltip, this)
          .off('mouseout', this.closeTooltip, this)
          .off('remove', this.closeTooltip, this)
...
sgmap.Tooltip = L.Popup.extend({
  ...
  _initLayout: function () {
    ...
    var outer = this._outer = L.DomUtil.create('div', prefix + "-outer", container);

    var wrapper = this._wrapper =
            L.DomUtil.create('div', prefix + '-content-wrapper small', outer);
    L.DomEvent.disableClickPropagation(wrapper);
 ...
 _updatePosition: function () {
    if (!this._map) { return; }
    var pt = this._map.latLngToContainerPoint(this._latlng);
    var map_width = this._map._container.clientWidth;
    var map_height = this._map._container.clientHeight;

    var div_width = $(this._outer).width();
    var div_height = $(this._outer).height();

    //TODO: Get rid of some of these numbers.
    var arrow_space = 13;

    //Because IE decided to not implement pointer-events:none
    var shim_space = 22;

    //Give priority to moving to side
    if (pt.x < div_width/2) {
      this._arrowDirection = "left";
    }
    else if (pt.x > (map_width - div_width/2)) {
      this._arrowDirection = "right";
    }
    else if (pt.y > div_height + 2*arrow_space) {
      this._arrowDirection = "bottom";
    }
    else {
      this._arrowDirection = "top";
    }
    ...
    if (this._arrowDirection == "bottom") {
    //Add small offset to prevent popup from appearing under cursor and closing itself
      // $(".leaflet-popup-content-wrapper.small").css('margin-bottom', 13 + 'px');
      offset.y = -1*shim_space;
    }
    else if (this._arrowDirection == "top") {
      offset.y = div_height + arrow_space*2 + shim_space;
      $(".leaflet-popup-tip-container.small").css('bottom', offset.y + 'px');
      // $(".leaflet-popup-content-wrapper.small").css('margin-top', 13 + 'px');
    }
    else {
      offset.y = Math.round(div_height/2.0) + arrow_space;
      $(".leaflet-popup-tip-container.small").css('margin-bottom', offset.y  + 'px');
    }

    this._outerBottom = -offset.y - (animated ? 0 : pos.y);

    if (this._arrowDirection == "left") {
      offset.x = shim_space;
      this._tip_border = L.DomUtil.create('div', 'leaflet-arrow small left border', this._outer);
      this._tip = L.DomUtil.create('div', 'leaflet-arrow small left inner', this._tip_border);

      $(".leaflet-popup-outer").css("-webkit-transform", "translate(-15px, 0px)");
      $(".leaflet-popup-outer").css("-moz-transform", "translate(-15px, 0px)");
      $(".leaflet-popup-outer").css("-o-transform", "translate(-15px, 0px)");
    }
    else if (this._arrowDirection == "right") {
      offset.x = -div_width - arrow_space*2 - shim_space;
      this._tip_border = L.DomUtil.create('div', 'leaflet-arrow small right border', this._outer);
      this._tip = L.DomUtil.create('div', 'leaflet-arrow small right inner', this._tip_border);
    ...
    else if (this._arrowDirection == "bottom") {
      offset.x = -(div_width/2.0) - arrow_space;
      this._tip_border = L.DomUtil.create('div', 'leaflet-arrow small bottom border', this._outer);
      this._tip = L.DomUtil.create('div', 'leaflet-arrow small bottom inner', this._tip_border);

      $(".leaflet-popup-outer").css("-webkit-transform", "translate(0, 15px)");
      $(".leaflet-popup-outer").css("-moz-transform", "translate(0, 15px)");
      $(".leaflet-popup-outer").css("-o-transform", "translate(0, 15px)");
    }
    else if (this._arrowDirection == "top") {
      offset.x = -(div_width/2.0) - arrow_space;
      this._tip_border = L.DomUtil.create('div', 'leaflet-arrow small top border', this._outer);
      this._tip = L.DomUtil.create('div', 'leaflet-arrow small top inner', this._tip_border);
      this._outer.insertBefore(this._tip_border, this._outer.firstChild);
    ...
    this._outerLeft = offset.x + (animated ? 0 : pos.x);

    // bottom position the popup in case the height of the popup changes (images loading etc)

    // this._container.style.top = -1 * Math.round(div_height/2.0) + 'px';
    this._outer.style.bottom = this._outerBottom + 'px';
    this._outer.style.left = this._outerLeft + 'px';
  },

  onRemove: function (map) {
    $(".leaflet-arrow.small").remove();
    map._panes.popupPane.removeChild(this._container);
